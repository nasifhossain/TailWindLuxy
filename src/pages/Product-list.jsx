        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6 mx-6 md:mx-12 mb-12">
          {data?.products?.length > 0 ? (
            data.products.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-xl shadow-md pb-4 flex flex-col w-full items-center justify-between hover:shadow-slate-500 transition-all duration-300 max-sm:h-[50vh]"
              >
                <div className="bg-slate-300 max-sm:h-[75%] rounded-t-xl w-full  flex flex-col justify-center items-center">
                  <img
